...
const ProductDetailScreen: React.FC<ProductDetailProps> = ({ productId }) => {
    ...
    const fetchAddress = async () => {
      if (
        ...
        try {
          ...
          if (isMounted)
            setAddress(
              `${product.location.coordinates[1].toFixed(4)}, ${product.location.coordinates[0].toFixed(4)}`,
            );
        }
  ...
  const handleScroll = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    if (
      sectionPositions.location > 0 &&
      scrollY >= sectionPositions.location - threshold
    ) {
      if (activeTab !== "location") setActiveTab("location");
    } else if (
      sectionPositions.details > 0 &&
      scrollY >= sectionPositions.details - threshold
    ) {
  ...
  const getSpecIcon = (
    specKey: string,
  ): { name: keyof typeof Ionicons.glyphMap; type: "ionicons" } => {
    ...
    if (
      key.includes("handcrafted") ||
      key.includes("handmade") ||
      key.includes("craft")
    ) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-bg_primary" edges={["top"]}>
      <ScrollView
        ref={mainScrollRef}
        className="flex-1"
                ...
                return (
                  ...
                  >
                    ...
                    <View className="items-center">
                      <Text
                        className="text-xs text-bg_black mb-1 text-center"
                        numberOfLines={2}
                        style={{ lineHeight: 14 }}
                      >
                        {displayKey}
                      ...
                      <Text
                        className="text-sm font-bold text-bg_black text-center"
                        numberOfLines={2}
                        style={{ lineHeight: 18 }}
                      >
                        {displayValue}
      ...
      {product?.seller && (
        <View className="absolute bottom-0 left-0 right-0 flex-row items-center px-4 py-3 bg-bg_white border-t border-border_primary">
          ...
          <View className="flex-1">
            ...
            <Text className="text-sm text-text_tertiary">Seller ⭐ 4.4</Text>
          </View>
