...
const getStableKey = (item: any): string => {
  return item?.key || item?.slug || normalizeFieldKey(item?.field || item?.label || "");
};
...
const extractQuestions = (payload: any): Question[] => {
  ...
  return questionsList
    ...
    .map((q) => ({
      ...q,
      options: Array.isArray(q.options) ? q.options.filter((o: any) => o != null) : [],
    }));
};
...
const normalizeSpecs = (specs: Record<string, any> | any[] = {}): Record<string, any> => {
  ...
  Object.entries(specs).forEach(([key, spec]) => {
    if (!spec) return;
    const value = typeof spec === "object" && "value" in spec ? spec.value : spec;
    if (isValidSpecValue(value)) {
      const stableKey = typeof spec === "object" ? getStableKey({ ...spec, key }) : key;
...
export const usePostAdAI = () => {
  ...
  const fetchFinal = useCallback(async ({
    ...
    try {
      ...
      const missingSpecs = (Array.isArray(intermediate?.specs) ? intermediate.specs : [])
      ...
      const questionsData = extractQuestions(finalPayload).filter((q) => 
  ...
  const fetchIntermediate = useCallback(async (basic: any, currentGenerationId: number, signal: AbortSignal) => {
    if (currentGenerationId !== generationIdRef.current || signal.aborted) return;

    try {
      const res = await post(PostAdApi.intermediatePreview, {
        subcategoryId: basic?.subcategory?.id || basic?.subcategory?._id || basic?.subcategoryId,
      ...
      Object.entries(intermediate).forEach(([key, value]) => {
        const skipKeys = ["slug", "images", "questions", "specs", "specifications", "category", "subcategory", "division", "title", "description", "details", "enhanced_description"];
