...
export const getProductImageUrl = (
  ...
  if (first) {
    const url =
      typeof first === "string"
        ? first
        : (first?.url ?? first?.link ?? first?.src);
...
const extractSellerInfo = (
  ...
  let sellerVerified: boolean | undefined =
    p.seller?.verified || p.seller?.isVerified || false;
  ...
  if (!sellerName && p.user && typeof p.user === "object")
    extractFromObject(p.user);
  if (!sellerName && p.createdBy && typeof p.createdBy === "object")
    extractFromObject(p.createdBy);
